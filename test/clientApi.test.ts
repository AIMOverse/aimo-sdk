 ...
 *
 * Tests the AimoClient API using SDK components.
 */
...
describe("Client API Tests", function () {
  // Increase timeout for API calls
  this.timeout(60000);

  describe("SVM Client", function () {
    ...
    it("should query session balance", async function () {
      ...
      assert.property(
        balance,
        "balance_usd",
        "Expected balance_usd in response"
      );
      assert.typeOf(
    ...
    it("should make chat completion request", async function () {
      ...
      if (response.status === 200) {
        ...
        console.log(
          `    Chat completion: ${body.choices[0]?.message?.content}`
        );
      } else {
        const body: any = await response.json();
